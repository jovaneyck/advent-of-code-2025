...
let rec connect (state : State) lookup =
    ...
    let circuits = (state |> List.except [c1;c2] |> List.append [combined])
    if circuits.Length = 1 then
        next
    else
        connect circuits rest
...
let lastConnection = connect initial lookup

let (((x1,_,_),(x2,_,_)),_) = lastConnection
x1 * x2
